...
export const Sidebar = (props) => {
  ...
  const deleteProject = (e) => {
    for (let project of Object.values(allProjects.projectObj)) {
      if (
        project.hash ===
        +e.target.nextElementSibling.getAttribute("project-key")
      ) {
    ...
    <div className="sidebar-grid">
      {Object.keys(allProjects.projectObj).map((project, index) => (
        <div key={index} className="project-x">
          <span onClick={deleteProject} onMouseOver={pushX} onMouseOut={popX}>
            &times;
          </span>
          <button
            ...
            project-key={allProjects.projectObj[project].hash}
          >
            {project}
          </button>
        </div>
